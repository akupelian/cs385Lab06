...
func getSessionLookup() func(string) (string, bool) {
	...
	if models.CassandraEnabled {
		return func(session string) (string, bool) {
			log.Print("Cassandra enabled function in lookup")
			var username string
			// UPDATE REQUIRED
			// Add logic to get a list of sessions from cassandra
			if err := models.CassandraSession.Query(`SELECT username FROM minibank.sessions WHERE session = ? LIMIT 1`, session).Consistency(gocql.One).Scan(&username); err != nil {
				log.Fatal(err)
			}
	...
	if models.CassandraEnabled {
		return func(session uuid.UUID, username string) {
			...
			if err := models.CassandraSession.Query(`INSERT INTO minibank.sessions (session, username, expiration) VALUES (?, ?, ?)`,
				session.String(), username, uint64(time.Now().UnixNano()/1000000) + sessionDuration).Exec(); err != nil {
					log.Fatal(err)
				}
		}
	} else {
...
func getSessionListLookup() func(string) UserSessions {
	...
	if models.CassandraEnabled {
		return func(username string) UserSessions {
			...
			var session string
			iter := models.CassandraSession.Query(`SELECT session from minibank.sessions WHERE username = ?`, username).Iter()
